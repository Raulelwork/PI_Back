        ...
        <Layout v-if="$page.props.auth.user.tipo == 'empresa'">
            ...
            <div class=" my-12 flex justify-center items-center flex-col  text-center text-black min-[600px]:mx-48">
                ...
                <table class="min-[600px]:min-w-full border-collapse block md:table">
                    ...
                    <tbody class="block md:table-row-group" v-for="f in filteredFiestas()" :key="f.id">
                        <tr  v-for="entr in f.entrada.filter(entr => entr.eliminado == 0)" :key="entr.id">
                            <td class="td"><span class="span">Email</span> {{ entr.tipo}}</td>
                            <td class="td"><span class="span">Fecha</span>{{ f.fecha }}</td>
                            <td class="td"><span class="span">Empresa</span>{{ f.empresa.nombre }}</td>
                            <td class="td">
                                <button @click="eliminarentrada(f.entrada.id)">
                                    <img src="../../img/icon/borrar.png" class="w-6 m-auto" alt="">
                                </button>
                            </td>
                        </tr>
                ...
                <div class="flex justify-center">
                    <button class="px-2 py-1 rounded-lg bg-gray-200 mr-2" :disabled="currentPage == 1"
                        @click="currentPage--">Anterior</button>
                    <button class="px-2 py-1 rounded-lg bg-gray-200" :disabled="filteredFiestas().length < pageSize"
...
export default {
    ...
    methods: {
        filteredFiestas() {
            ...
            return this.fiestas.filter(f => f.empresa.id_usuario == this.$page.props.auth.user.id)
                .slice(start, end);
        },
